...
const ArticleDetail: React.FC<{ params: { slug: string } }> = async ({ params }) => {
  const articleData = await getArticleData(params.slug);
  ...
  return (
    <Layout>
      <article className="py-8 px-4 bg-white rounded-lg shadow-xl mb-12">
        ...
        <div
          className="prose prose-blue max-w-none text-gray-800 leading-relaxed ml-[10%] mr-[10%] overflow-hidden text-clip" // Gunakan 'prose' dari @tailwindcss/typography jika diinstal
